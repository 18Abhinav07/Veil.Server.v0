...
deploy_contract() {
  ...
  while true; do
    ...
    # Retry on transient sequence / timeout errors (testnet congestion)
    if echo "$output" | grep -qE 'TxBadSeq|timeout|timed out'; then
      attempts=$((attempts + 1))
      [[ $attempts -lt 5 ]] || { echo "$output" >&2; die "failed to deploy $name after $attempts attempts"; }
      echo "==> [retry $attempts] $name: transient error, waiting 10s…" >&2
      sleep 10
      continue
    fi
    break
  done
  # Wait one ledger for sequence to settle before next deployment
  sleep 8
  local id
