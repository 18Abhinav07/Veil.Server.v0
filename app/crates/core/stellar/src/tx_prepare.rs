...
use crate::{
    ...
    conversions::field_to_scval_u256,
    soroban_encode::{
...
impl StateFetcher {
    ...
    /// Simulates `insert_leaf` on the configured ASP membership contract and
    /// returns unsigned XDR + auth entries for the admin/service account.
    pub async fn prepare_asp_membership_insert(
        &self,
        source_account: &str,
        leaf: Field,
    ) -> Result<PreparedSorobanTx> {
