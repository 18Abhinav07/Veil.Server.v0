...
// ---------------------------------------------------------------------------
// BulkWithdrawSession — sequential multi-recipient payout from a single note
// ---------------------------------------------------------------------------

/// One recipient in a bulk payment.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BulkWithdrawRecipient {
    /// Stellar address or contract id that will receive the tokens.
    pub recipient_address: String,
    /// Amount to send (must be positive, stored as ExtAmount for API
    /// consistency; internally converted to NoteAmount for arithmetic).
    pub amount: ExtAmount,
}

/// Per-step Merkle witness that the caller must supply for each payment.
///
/// The change note from step N is inserted into the pool tree at N+1 before
/// step N+1 starts; the caller must supply the updated proof.
#[derive(Clone, Debug)]
pub struct StepMerkleWitness {
    /// Updated pool Merkle root (after the previous step was confirmed).
    pub pool_root: Field,
    /// Sibling hashes for the current change note commitment.
    pub merkle_path_elements: Vec<Field>,
    /// Packed path indices for the current change note commitment.
    pub merkle_path_indices: Field,
}

/// Session that chains N sequential withdraw calls, each spending the change
/// note left by the previous step.
///
/// Construction validates that the sum of payment amounts does not exceed the
/// initial note balance and pre-generates deterministic change blindings so the
/// caller can reconstruct the next step's change note without re-querying the
/// session.
///
/// Usage:
/// ```ignore
/// let mut session = BulkWithdrawSession::new(...)?;
/// while !session.is_done() {
///     let witness = StepMerkleWitness { pool_root, ... }; // fetch from indexer
///     let params = session.build_step(witness)?;
///     let artifact = withdraw(params, hash_ext_data)?;
///     // submit artifact → wait for confirmation → update pool root
///     session.advance();
/// }
/// ```
pub struct BulkWithdrawSession {
    ...
    tree_depth: u32,
    smt_depth: u32,
    recipients: Vec<BulkWithdrawRecipient>,
    /// Pre-generated change blindings — one per step (for each step's change
    /// output, which becomes the next step's input).
    change_blindings: Vec<Field>,
    /// Current step index (0-based).
    current_step: usize,
    /// Blinding of the note that is the current step's input.
    current_note_blinding: Field,
    /// Amount in the current step's input note.
    current_note_amount: NoteAmount,
}

impl BulkWithdrawSession {
    /// Create a new session.
    ///
    /// - `initial_note_blinding` — blinding of the note to be spent in step 0.
    /// - `initial_note_amount` — balance of that note.
    /// - `recipients` — ordered list of payouts; must not be empty, and the sum
    ///   of amounts must not exceed `initial_note_amount`.
    pub fn new(
        priv_key: NotePrivateKey,
        encryption_pubkey: EncryptionPublicKey,
        initial_note_blinding: Field,
        initial_note_amount: NoteAmount,
        recipients: Vec<BulkWithdrawRecipient>,
        membership_proof: AspMembershipProof,
        non_membership_proof: AspNonMembershipProof,
        tree_depth: u32,
        ...
        if recipients.is_empty() {
            return Err(anyhow!("BulkWithdrawSession requires at least one recipient"));
        }

        // Validate that sum of amounts ≤ initial balance.
        let mut running = initial_note_amount;
        for (i, r) in recipients.iter().enumerate() {
            let payout = NoteAmount::try_from(r.amount).map_err(|_| {
                anyhow!("recipient[{i}].amount must be positive and fit in NoteAmount")
            })?;
            running = running.checked_sub(payout).ok_or_else(|| {
                anyhow!(
                    "sum of recipient amounts exceeds initial note balance at recipient[{i}]"
                )
            })?;
        }

        // Pre-generate change blindings for each step.
        let mut change_blindings = Vec::with_capacity(recipients.len());
        for _ in 0..recipients.len() {
            change_blindings.push(encryption::generate_random_blinding()?);
        }
        ...
        Ok(Self {
            ...
            change_blindings,
            current_step: 0,
            current_note_blinding: initial_note_blinding,
            current_note_amount: initial_note_amount,
        })
    }

    /// Build `WithdrawParams` for the current step.
    ///
    /// The returned params include explicit `outputs` so the change blinding
    /// is the one tracked by this session (enabling the caller to later
    /// reconstruct the change note's commitment and Merkle proof).
    pub fn build_step(&self, witness: StepMerkleWitness) -> Result<WithdrawParams> {
        if self.is_done() {
            return Err(anyhow!("BulkWithdrawSession is already complete"));
        }

        let step = self.current_step;
        let recipient = &self.recipients[step];
        let withdraw_amount = recipient.amount;
        let payout_note = NoteAmount::try_from(withdraw_amount)
            .map_err(|_| anyhow!("step {step}: withdraw_amount must be positive NoteAmount"))?;
        let change_amount = self
            .current_note_amount
            .checked_sub(payout_note)
            .ok_or_else(|| {
                anyhow!(
                    "step {step}: insufficient change note balance ({} < {})",
                    self.current_note_amount,
                    payout_note
                )
            })?;

        let change_blinding = self.change_blindings[step];

        let input = TransactInputNote {
            amount: self.current_note_amount,
            blinding: self.current_note_blinding,
            merkle_path_elements: witness.merkle_path_elements,
            merkle_path_indices: witness.merkle_path_indices,
        };

        // Dummy output slot: zero amount, random blinding (auto-encrypted to self).
        let dummy_blinding = encryption::generate_random_blinding()?;
        let outputs = vec![
            TransactOutput {
        ...
        Ok(WithdrawParams {
            ...
            pool_root: witness.pool_root,
            withdraw_recipient: recipient.recipient_address.clone(),
            withdraw_amount,
            inputs: vec![input],
            outputs: Some(outputs),
            membership_proof: self.membership_proof.clone(),
            non_membership_proof: self.non_membership_proof.clone(),
            tree_depth: self.tree_depth,
            smt_depth: self.smt_depth,
        })
    }

    /// Advance to the next step after the current step's transaction is
    /// confirmed on-chain.
    ///
    /// Updates the internal change note state: the change blinding and amount
    /// from the just-completed step become the input for the next step.
    pub fn advance(&mut self) -> Result<()> {
        if self.is_done() {
            return Err(anyhow!("BulkWithdrawSession is already complete"));
        }
        let step = self.current_step;
        let payout_note = NoteAmount::try_from(self.recipients[step].amount)
        ...
        self.current_note_blinding = self.change_blindings[step];
        self.current_note_amount = new_amount;
        self.current_step = self.current_step.saturating_add(1);
        Ok(())
    }

    /// Returns `true` when all recipients have been paid.
    pub fn is_done(&self) -> bool {
        self.current_step >= self.recipients.len()
    }
    ...
    /// The blinding of the current step's input change note.
    ///
    /// Use this to locate the commitment in the pool tree and build the Merkle
    /// proof for `build_step`.
    pub fn current_note_blinding(&self) -> &Field {
        &self.current_note_blinding
    }

    /// The amount of the current step's input change note.
    pub fn current_note_amount(&self) -> NoteAmount {
        self.current_note_amount
    }

    /// The pre-generated change blinding for the output of the given step.
    ///
    /// Use this to reconstruct the change commitment after the step confirms
    /// (needed to build the Merkle proof for the following step).
    pub fn change_blinding_for_step(&self, step: usize) -> Option<&Field> {
        self.change_blindings.get(step)
    }
}
